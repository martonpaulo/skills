---
name: review-changes
description: Review the working diff against a fixed point on two independent axes, standards and intent, before the change becomes a pull request. Use only when the user explicitly invokes $review-changes for uncommitted work, staged work, a local branch, or a range such as "review since main". Not for reviewing a pull request, which belongs to issue-review, and not for adversarial bug hunting, implementing the findings, or reviewing an untouched codebase.
