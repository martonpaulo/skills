...
def _user_protected_paths() -> List[str]:
    """
    Paths under the user's home directory that must never be deleted.

    These hold credentials, keys, personal libraries, cloud-synced data, and
    irreplaceable backups. Deleting them is not recoverable by reinstalling a
    package or rebuilding a project, so they are protected unconditionally.
    """
    relative = [
        ...
        ".npmrc",
        "Library/Keychains",
        # Personal libraries and irreplaceable data
    ...
    return [os.path.join(home, item) for item in relative]
