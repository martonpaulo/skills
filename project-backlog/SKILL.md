...
3. Use the repository's existing metadata taxonomy. Do not create a new label system without explicit authorization. When the repository has no convention of its own, audit against the taxonomy `issue-capture` documents: closed value sets for `type:`, `priority:` and `effort:`, an open set for `area:`.
