...
Every issue body and every comment published here ends with the agent signature line defined in [`github-conventions`](../github-conventions/SKILL.md), for example `🤖 AI-generated by ❋ Claude Sonnet 5 (High)`. Unsigned published text is a defect, including a one-line comment.

Pass the body through stdin as above rather than writing a draft to disk. When a draft file is genuinely needed, it lives in a uniquely named system temporary directory and is deleted before this run reports completion, per [`github-conventions`](../github-conventions/SKILL.md). The exception is the ready-to-post draft produced when GitHub is unreachable, which is the output and stays.
