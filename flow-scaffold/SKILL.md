...
This runs before `flow-setup-project`, which then aligns the generated repository with the owner's
identity, policy, licensing, and documentation. Keeping them separate is deliberate:
`flow-setup-project` owns policy and explicitly refuses to scaffold, so that a setup run against a
...
2. Establish what is being built from `flow-define-product` when it has run, or from the owner
...
8. Report the tree, the commands available, and what `flow-setup-project` still has to decide.
   Do not commit unless the owner asks; the repository has no commit policy yet, because that is
   `flow-setup-project`'s job.
...
one of those belongs to `flow-setup-project`, which runs next.

Generated code is a starting point, not a contract. Do not treat the scaffolder's conventions as
rules this repository must follow forever; `flow-setup-project` and the repository's own guidance
decide that.
